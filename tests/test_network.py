...
class TestPoliteness:
    ...
    def test_thread_safe_spacing_between_workers(self, clock, monkeypatch):
        """Two concurrent workers still space their zameen dispatches apart."""
        ScriptedTransport(monkeypatch, [(200, {}), (200, {})])
        dispatch_times: list[float] = []
        original_make_request = HTTPConnectionPool._make_request

        def timing_make_request(*args, **kwargs):
            dispatch_times.append(clock.monotonic())
            return original_make_request(*args, **kwargs)

        monkeypatch.setattr(
            HTTPConnectionPool, "_make_request", timing_make_request
        )
        ...
        # Ordering of the two dispatch timestamps is NOT assertable here:
        # they are read off the VIRTUAL clock outside the politeness lock, so
        # thread scheduling decides whether a mark lands before or after the
        # other worker's (virtual) sleep advances the clock. The deterministic
        # contract instead: both dispatches happen, and exactly ONE full-delay
        # wait was enforced under the lock.
